...
public class LevelDBCacheStoreConfig extends LockSupportCacheStoreConfig {
   ...
   /**
    * @configRef desc="Whenever a new entry is stored, an expiry entry is created and added to the a
    *            queue that is later consumed by the eviction thread. This parameter sets the size
    *            of this queue."
    */
   protected int expiryQueueSize = DEFAULT_EXPIRY_QUEUE_SIZE;

   /**
    * @configRef desc="There are two methods to clear all entries in LevelDB. One method is to
    *            iterate through all entries and remove each entry individually. The other method is
    *            to delete the database and re-init. For smaller databases, deleting individual
    *            entries is faster than the latter method. This configuration sets the max number of
    *            entries allowed before using the latter method."
    */
